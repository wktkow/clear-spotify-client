...
def generate_web_bridge_css(colors: dict[str, str]) -> str:
    ...
    return "\n".join([
        ...
        "/* When the panel is open (.Vmaq2JjgsQ0qt9VG is present), Spotify's */",
        "/* own CSS already resets these – but we reinforce with our selectors. */",
        ".Root__right-sidebar > div {",
        "  width: 100% !important;",
        "}",
        "/* Inner positioning wrapper – neutralize absolute + transform */",
        ...
        f"  --fg-color: rgba({rgb_text}, 1) !important;",
        f"  --bg-color: rgba({rgb_text}, 0.125) !important;",
        f"  --is-active-fg-color: rgba({rgb_text}, 0.75) !important;",
        "}",
        "",
        ...
        f"  background-color: {text} !important;",
        "}",
        "",
        ...
        "}",
        "/* Compact cover art */",
        '#Desktop_LeftSidebar_Id [data-encore-id="listRow"] img,',
